...
import com.px.dms.model.WfDocumentTypeModel;
        ...
        try {
            ...
            try{
                if(documentTypeModel.getParentId()>0){
                    WfDocumentType parentType = documentTypeService.getByIdNotRemoved(documentTypeModel.getParentId());
                    documentType.setDocumentTypeDetail(parentType.getDocumentTypeName()+"_"+documentTypeModel.getName());
                }
            }catch(Exception eee){}
             documentType.setNodeLevel(documentTypeModel.getNodeLevel());
            documentType.setParentKey(documentTypeModel.getParentKey());
            documentType.setIsFolder(documentTypeModel.getIsFolder());
    ...
    public Response update(
        ...
        try {
            ...
            if (documentType != null) {
                ...
                documentTypeService.updateDocumentCodeChildByParent(listUpdateData, new ArrayList<Integer>(Arrays.asList(documentType.getId())), new ArrayList<String>(Arrays.asList(documentType.getDocumentTypeCode())), new ArrayList<String>(Arrays.asList(documentType.getDocumentTypeName())));
                status = Response.Status.OK;
                responseData.put("data", documentTypeService.tranformToModel(documentType));
                responseData.put("message", "");
            }
...
//            }
    ...
    @ApiOperation(
            value = "Method for list WfDocumentType by wfTypeCode",
            notes = "ขอข้อมูลประเภทเอกสาร ด้วย document_type_code",
            response = WfDocumentTypeModel.class
    )
    @ApiResponses({
        @ApiResponse(code = 200, message = "WfDocumentType by id success.")
    ...
    @GET
    @Consumes({MediaType.APPLICATION_JSON})
    @Path(value = "listByWfTypeCode/{wfTypeCode}")
    public Response getByWfTypeCode(
            @BeanParam VersionModel versionModel,
            @ApiParam(name = "wfTypeCode", value = "wfTypeCode", required = true)
            @PathParam("wfTypeCode") String wfTypeCode
    ) {
        LOG.debug("getById...");
        LOG.debug("wfTypeCode = " + wfTypeCode);
        Gson gs = new GsonBuilder()
                .setVersion(versionModel.getVersion())
                .excludeFieldsWithoutExposeAnnotation()
                .disableHtmlEscaping()
        ...
        try {
            WfDocumentTypeService documentTypeService = new WfDocumentTypeService();
            ArrayList<WfDocumentTypeModel> listWfDocumentTypeModel = new ArrayList<>();
            List<WfDocumentType> listDocumentType = documentTypeService.listWfTypeByTypeCode(wfTypeCode);
//            List<WfDocumentType> listDocumentType = documentTypeService.listChildByParentId(listOptionModel.getSort(), listOptionModel.getDir(), parentId);
            if (!listDocumentType.isEmpty()) {
                for (WfDocumentType documentType : listDocumentType) {
                    listWfDocumentTypeModel.add(documentTypeService.tranformToModel(documentType));
