...
import com.px.dms.daoimpl.WfDocumentTypeDaoImpl;
...
public class WfDocumentTypeService implements GenericTreeService<WfDocumentType, WfDocumentTypeModel> {
    ...
    public WfDocumentTypeModel tranformToModel(WfDocumentType documentType) {
        ...
        if (documentType != null) {
            ...
            documentTypeModel.setIsFolder(documentType.getIsFolder());
        }
    ...
    public void updateDocumentCodeChildByParent(List<List<WfDocumentType>> listChild, List<Integer> parentId, List<String> parentDocCode, List<String> parentName) {
        ...
        for (int i = 0; i < listChild.size(); i++) {
            for (int j = 0; j < listChild.get(i).size(); j++) {
                ...
                listChild.get(i).get(j).setDocumentTypeDetail(parentName.get(i)+"_"+listChild.get(i).get(j).getDocumentTypeName());
                ...
                listParentName.add(listChild.get(i).get(j).getDocumentTypeName());
            }
        }
        if (!newChild.isEmpty()) {
            updateDocumentCodeChildByParent(newChild, listParentId, listParentCode,listParentName);
        }
    }

    public WfDocumentType getwftypeByname(String wfTypeName) {
        return documentTypeDaoImpl.getwftypeByname(wfTypeName);
    }

    public List<WfDocumentType> listWfTypeByTypeCode(String wfTypeCode) {
        return documentTypeDaoImpl.listWfTypeByTypeCode(wfTypeCode);
    }
}
