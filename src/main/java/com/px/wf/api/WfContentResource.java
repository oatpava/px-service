...
//            wfContent.setWfContentBookYear(yearToday);
    ...
    @ApiOperation(
            value = "สร้างข้อมูลประวัติการใช้งานระบบ เปิดอ่านหนังสือ",
            notes = "สร้างข้อมูลประวัติการใช้งานระบบ เปิดอ่านหนังสือ",
            response = WfContentModel.class
    )
    @ApiResponses({
    ...
    public Response createLogOpen(
        ...
        try {
            ...
            responseData.put("success", true);
            responseData.put("message", "LogData created successfully.");
        } catch (Exception ex) {
            LOG.error("Exception = " + ex.getMessage());
            responseData.put("errorMessage", ex.getMessage());
        }
    ...
    @ApiOperation(
            value = "สร้างข้อมูลประวัติการใช้งานระบบ เปิดอ่านเอกสาร",
            notes = "สร้างข้อมูลประวัติการใช้งานระบบ เปิดอ่านเอกสาร",
            response = WfContentModel.class
    )
    @ApiResponses({
        @ApiResponse(code = 201, message = "LogData created successfully."),
    ...
    @POST
    @Path(value = "/logViewFile")
    @Consumes({MediaType.APPLICATION_JSON})
    public Response createLogViewFile(
            @BeanParam VersionModel versionModel,
            WfContentModel wfContentModel
    ) {
        LOG.info("createLogViewFile...");
        Gson gs = new GsonBuilder()
                .setVersion(versionModel.getVersion())
                .excludeFieldsWithoutExposeAnnotation()
                .disableHtmlEscaping()
        ...
        try {
            WfContentService wfContentService = new WfContentService();
            wfContentModel = wfContentService.saveLogForViewFile(wfContentModel, httpHeaders.getHeaderString("clientIp"), Integer.parseInt(httpHeaders.getHeaderString("userID")));
            status = Response.Status.OK;
            responseData.put("data", wfContentModel);
            responseData.put("success", true);
            responseData.put("message", "LogData created successfully.");
    ...
    @ApiOperation(
            value = "สร้างข้อมูลประวัติการใช้งานระบบ สำเนาเอกสาร",
            notes = "สร้างข้อมูลประวัติการใช้งานระบบ สำเนาเอกสาร",
            response = WfContentModel.class
    )
    @ApiResponses({
        @ApiResponse(code = 201, message = "LogData created successfully."),
    ...
    @POST
    @Path(value = "/logDownloadFile")
    @Consumes({MediaType.APPLICATION_JSON})
    public Response createLogDownloadFile(
            @BeanParam VersionModel versionModel,
            WfContentModel WfContentModel
    ) {
        LOG.info("createLogDownloadFile...");
        Gson gs = new GsonBuilder()
                .setVersion(versionModel.getVersion())
                .excludeFieldsWithoutExposeAnnotation()
                .disableHtmlEscaping()
        ...
        try {
            WfContentService wfContentService = new WfContentService();
            WfContentModel = wfContentService.saveLogForDownloadFile(WfContentModel, httpHeaders.getHeaderString("clientIp"), Integer.parseInt(httpHeaders.getHeaderString("userID")));
            status = Response.Status.OK;
            responseData.put("data", WfContentModel);
            responseData.put("success", true);
            responseData.put("message", "LogData created successfully.");
